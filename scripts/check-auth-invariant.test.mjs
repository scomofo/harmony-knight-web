...
test("reads the value a live dev server resolved", async () => {
  assert.equal(
    await probeDevAuthEnabled("http://127.0.0.1:8086", appEnvFetch({ VITE_AUTH_ENABLED: "false" })),
    false,
  );
});

test("a server started without the flag reads as sign-in on", async () => {
  assert.equal(await probeDevAuthEnabled("http://127.0.0.1:8086", appEnvFetch({})), true);
});
...
test("the build side resolves app-env from an isolated workspace", () => {
  const root = mkdtempSync(join(tmpdir(), "auth-invariant-env-"));
